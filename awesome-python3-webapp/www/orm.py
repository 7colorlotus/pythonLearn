#目前要顺利编译通过，必须安装使用aiomysql0.0.7版本。下载地址：https://pypi.python.org/pypi/aiomysql/0.0.7。

#安装操作步骤：
#1. 卸载mysql-connector-python、aiomysql、PyMySQL，使用pip重新安装aiomysql； 
#2. aiomysql版本为0.0.7；
#3. PyMySQL版本为0.6.7；
#4. 使用aiomysql连接数据库正常。
		...
		for k,v in attrs.items():
			if isinstance(v,Field):
				...
				if v.primary_key:
					...
					fields.append(k)
		if not primaryKey:
			raise StandardError('Primary key not fund.')
...
class Model(dict,metaclass=ModelMetaclass):
	...
	async def findNumber(cls,selectField,where=None,args=None):
		...
		sql = ['selet %s _num_ from `%s`' % (selectField,cls.__table__)]
		if where:
			sql.append('where')
			sql.append(where)
